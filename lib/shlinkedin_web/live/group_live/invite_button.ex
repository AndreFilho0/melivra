defmodule ShlinkedinWeb.GroupLive.InviteButton do
  ...
  def render(assigns) do
    ...
    <% "Invite" -> %> <button phx-click="invite" phx-value-id="<%= @match.id %>" phx-target="<%= @myself %>"
        class="inline-flex items-center px-3 py-1 border border-transparent shadow-sm text-xs font-semibold rounded-full text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Convidar</button>
    <% end %>
    </div>
    """
  end

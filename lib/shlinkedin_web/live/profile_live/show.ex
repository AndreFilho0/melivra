defmodule ShlinkedinWeb.ProfileLive.Show do
  ...
  defp apply_action(socket, :new_testimonial, %{"slug" => slug}) do
    socket
    |> assign(:page_title, "Fazer um review para  #{socket.assigns.show_profile.persona_name}")
    |> assign(:from_profile, socket.assigns.profile)
    |> assign(:to_profile, Profiles.get_profile_by_slug(slug))
  ...
  defp apply_action(socket, :show_intern, %{"id" => id, "status" => status}) do
    intern = Shlinkedin.Interns.get_intern!(id)

    socket
    |> assign(intern: intern, status: status)
  end
  ...
  defp apply_action(socket, :show_intern, %{"id" => id}) do
    ...
    socket
    |> assign(intern: intern, status: nil)
  end
